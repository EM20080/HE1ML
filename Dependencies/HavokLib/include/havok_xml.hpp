...
public:
  ...
  template <class C, class... Args> C *NewClass(Args &&...args) {
    C *cls = new C(std::forward<Args>(args)...);
    classes.emplace_back(cls);
    return cls;
  }
};
...
class xmlRootLevelContainer : public hkRootLevelContainerInternalInterface {
  ...
  void AddVariant(IhkVirtualClass *input, std::string_view name = {},
                  std::string_view variantClassName = {}) {
    std::string_view className = variantClassName;
    if (className.empty()) {
      className = checked_deref_cast<const hkVirtualClass>(input)->className;
    }
    if (name.empty()) {
      name = className;
    }
    strings.emplace_back(name);
    strings.emplace_back(className);
    variants.push_back({strings[strings.size() - 2],
                        strings[strings.size() - 1], input});
  }
