...
#define LIBGENS_MODEL_ROOT_DYNAMIC_HE2_2          6
#define LIBGENS_MODEL_ROOT_DYNAMIC_FORCES         50
#define LIBGENS_MODEL_ROOT_DYNAMIC_FRONTIERS      51
#define LIBGENS_MODEL_ROOT_DYNAMIC_SHADOW         52
#define LIBGENS_MODEL_ROOT_DYNAMIC_SHADOW_RAW     53
#define LIBGENS_MODEL_ROOT_DYNAMIC_GENERATIONS_2024 54

#define LIBGENS_MODEL_SAMPLE_CHUNK_DEFAULT        0
#define LIBGENS_MODEL_SAMPLE_CHUNK_HE2            1
...
namespace LibGens {
	...
	class Model {
		...
		public:
			...
			void save(string filename_p, vector<Model *> lod_models, vector<unsigned char> lod_cascades, vector<float> lod_distances, int root_type = LIBGENS_MODEL_ROOT_DYNAMIC_FRONTIERS);
			void read(File *file);
			void readSkeleton(File *file);
			void readSampleChunkHeader(File* file);
			void write(File* file);
			void writeGenerations2024(File* file);
			void writeSampleChunkHeader(File* file, int root_type = LIBGENS_MODEL_ROOT_DYNAMIC_GENERATIONS, int sample_chunk_type = LIBGENS_MODEL_SAMPLE_CHUNK_DEFAULT);
			void buildAABB();
			AABB getAABB();
